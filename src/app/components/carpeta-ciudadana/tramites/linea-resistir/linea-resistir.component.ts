...
export class LineaResistirComponent implements OnInit {
    ...
    // Se debe validar a donde enviar los campos que no se envian en la regla
    // paisDomicilioSocial y provinciaDomicilioSocial se esta tomando desde los campos de detail-cost-financed
    // se debe validar la funcion getDecision, se comenta esta funcion ya que no se tienen todos los datos requeridos
    // autor: @William
    private checkBusinessRulesAddress() {
        const ruleBody:BusinessRuleBodyAddress = {
          paisDomicilioSocial: this.formLineaResistir.value.contact_data_country,
          provinciaDomicilioSocial: this.formLineaResistir.value.contact_data_province || "",
          paisLocalidadProyecto: this.formLineaResistir.value.contact_data_country,
          provinciaLocalidadProyecto: this.formLineaResistir.value.contact_data_province || ""
        };
