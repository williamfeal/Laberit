...
export class DetailCostFinancedComponent implements OnInit {
    ...
    // se cargan los paises desde el servicio y se setea el pais españa por defecto
    // autor: @william
    public getCountries() {
      this.catalogsService.getCatalogByCode(ConceptConstants.COUNTRIES).pipe(
        ...
        data => {
          ...
          this.formLineaResistir.patchValue({
            contact_data_country: 'countries-españa'
          })
          this.getSpainCountries()
        }
      )
    ...
    // se cargan las provincias de españa desde el servicio y se setea valencia por defecto
    // autor: @william
    public getSpainCountries() {
      this.catalogsService.getCatalogByCode(ConceptConstants.COUNTRIES_SPAIN).pipe(
        ...
        data =>{
          this.provincias = AppUtils.sortConceptsAlphabetically(data)
          console.log(this.provincias)
          this.formLineaResistir.patchValue({
            contact_data_province: 'countries-spain-provinces-46'
          })
          this.onChangeSpainCountry()
        } 
      )
    }
  
    // se cargan los municipios de valencia desde el servicio y se setea Valencia, Alicante y Castrillon por defecto
    // autor: @william
    public onChangeSpainCountry() {
      this.province = 'countries-spain-provinces-46';
      this.catalogsService.getCatalogByCode(this.province).pipe(
        takeUntil(this.unsubscribe$)
      ).subscribe(
        data => {
          const mpios =  AppUtils.sortConceptsAlphabetically(data)
          const arrayMpios: any = [
            {concept_code:'countries-spain-provinces-46-250'},
            {concept_code: 'countries-spain-provinces-3'},
            {concept_code: 'countries-spain-provinces-12'}
          ]
          console.log('----Mpios------',mpios)
          for(let i=0; i < arrayMpios.length; i++){
            console.log(arrayMpios[i])
            const mpio = mpios.find(x=> x.concept_code == arrayMpios[i].concept_code)
            console.log(mpio)
            if (mpio !== undefined){
